...
namespace :k8s_landing_page do
  ...
  desc "Start k8s-landing-page production test environment"
  task :prod_test_up do
    puts "Starting k8s-landing-page production test environment..."
    ...
    if $?.exitstatus == 0
      ...
      puts "Use 'bundle exec rake k8s_landing_page:prod_test_health' to check health"
      puts "Use 'bundle exec rake k8s_landing_page:prod_test_down' to stop"
    else
  ...
  desc "Stop k8s-landing-page production test environment"
  task :prod_test_down do
    puts "Stopping k8s-landing-page production test environment..."
  ...
  desc "Check health of k8s-landing-page production test environment"
  task :prod_test_health do
    puts "Checking k8s-landing-page production test health..."
    ...
    if result
      ...
      puts "Try running: bundle exec rake k8s_landing_page:prod_test_logs"
      exit 1
    end
  end
  
  desc "Show logs from k8s-landing-page production test environment"
  task :prod_test_logs do
    puts "Showing k8s-landing-page production test logs..."
  ...
  task :prod_test_cycle do
    puts "Running complete k8s-landing-page production test cycle..."
    
    begin
      # Stop any existing test environment
      Rake::Task["k8s_landing_page:prod_test_down"].invoke
      
      # Start production test environment
      Rake::Task["k8s_landing_page:prod_test_up"].invoke
      ...
      # Run health check
      Rake::Task["k8s_landing_page:prod_test_health"].invoke
      ...
      puts "Check logs with: bundle exec rake k8s_landing_page:prod_test_logs"
      exit 1
    ensure
  ...
  task :push_latest_to_ecr do
    puts "Pushing latest k8s-landing-page image to ECR..."
    
    # Configuration
    aws_profile = "ishiori1gp"
    ecr_registry = "public.ecr.aws/p1f1p2p3"
    ecr_repo = "ishiori-k8s-public-ecr"
    app_name = "k8s-landing-page"
    
    # Change to the k8s-landing-page directory
    k8s_landing_page_dir = File.expand_path("../../k8s-landing-page", __dir__)
    
    # Get current commit hash
    commit_hash = `cd #{k8s_landing_page_dir} && git rev-parse --short HEAD`.strip
    ...
    system("cd #{k8s_landing_page_dir} && docker build -t #{full_image_name} -f docker/Dockerfile .")
